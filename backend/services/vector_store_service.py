    ...
    def _index_to_chroma(self, embeddings_data: Dict[str, Any], config: VectorDBConfig) -> Dict[str, Any]:
        ...
        try:
            ...
            for emb in embeddings_data["embeddings"]:
                ...
                # 如果存在标题分块的额外字段，也加入（确保都是标量）
                if "title" in emb["metadata"]:
                    metadata["title"] = str(emb["metadata"]["title"])
                if "heading_level" in emb["metadata"]:
                    metadata["heading_level"] = int(emb["metadata"]["heading_level"])
                if "parent_title" in emb["metadata"]:
                    metadata["parent_title"] = str(emb["metadata"]["parent_title"])
