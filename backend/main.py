...
from services.structured_parser import StructuredParser
# ====== 在 main.py 最开头添加 ======
import sys
import nltk
import os

# 确保 NLTK 能找到本地 punkt_tab
custom_nltk_path = r'D:\nltk_data'   # 请替换为您的实际路径
nltk.data.path.insert(0, custom_nltk_path)
os.environ['NLTK_DATA'] = custom_nltk_path

# 阻止 unstructured 自动下载 NLTK 数据
import unstructured.nlp.tokenize
def _patched_download_nltk_packages():
    # 什么都不做，只是记录一下
    print("Skipping NLTK download (using local data)")
unstructured.nlp.tokenize.download_nltk_packages = _patched_download_nltk_packages
# 设置日志
...
async def chunk_document(data: dict = Body(...)):
    try:
        ...
        structured_doc_id = data.get("structured_doc_id")

        # 基础验证
        if not chunking_option:
            raise HTTPException(status_code=400, detail="Missing required parameter: chunking_option")

        chunking_service = ChunkingService()

        # 按标题切分
        if chunking_option == "by_headings":
            if not structured_doc_id:
                raise HTTPException(status_code=400, detail="Missing structured_doc_id for by_headings chunking")
            structured_path = os.path.join("03-structured-docs", structured_doc_id)
            if not os.path.exists(structured_path):
                raise HTTPException(status_code=404, detail="Structured document not found")
            with open(structured_path, "r", encoding="utf-8") as f:
                structured_doc = json.load(f)
            metadata = {
                "filename": structured_doc.get("metadata", {}).get("filename", ""),
            ...
            base_name = structured_doc.get("metadata", {}).get("filename", "doc").replace('.pdf', '')
...
async def parse_pdf_structured(
    file: UploadFile = File(...),
    strategy: str = Form("hi_res"),
    save_to_disk: bool = Form(True)
):
    import traceback
    temp_path = None
    try:
        ...
        elements = loader.load_pdf_structured(temp_path, strategy=strategy)

        parser = StructuredParser()
        structured_doc = parser.parse(elements, filename=file.filename)

        saved_path = None
        if save_to_disk:
            os.makedirs("03-structured-docs", exist_ok=True)
            out_filename = f"{file.filename.replace('.pdf', '')}_structured.json"
            saved_path = os.path.join("03-structured-docs", out_filename)
        ...
        logger.error(f"Error in parse-structured: {str(e)}\n{traceback.format_exc()}")
        raise HTTPException(status_code=500, detail=f"解析失败: {str(e)}")
    finally:
        if temp_path and os.path.exists(temp_path):
            os.remove(temp_path)

@app.get("/structured-docs")
async def list_structured_docs():
    """列出所有已解析的结构化文档"""
    try:
        docs_dir = "03-structured-docs"
        ...
        for filename in os.listdir(docs_dir):
            if filename.endswith('.json'):
                ...
                with open(file_path, 'r', encoding='utf-8') as f:
                    ...
                    docs.append({
                        "id": filename,
                        "name": data.get("metadata", {}).get("filename", filename),
                    })
        return {"documents": docs}
    except Exception as e:
        logger.error(f"Error listing structured docs: {str(e)}")
        raise HTTPException(status_code=500, detail=str(e))