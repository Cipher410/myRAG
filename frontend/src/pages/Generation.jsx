...
const Generation = () => {
  ...
  const saveToHistory = (query, response, provider, modelName, searchResults) => {
    const historyItem = {
  ...
  // 如果从搜索页面跳转过来，获取搜索结果
  useEffect(() => {
    if (location.state && location.state.query && location.state.results) {
  ...
  useEffect(() => {
    if (location.state && location.state.loadHistory) {
      const history = location.state.loadHistory;
      setQuery(history.query);
      setResponse(history.response);
      setProvider(history.provider);
      setModelName(history.modelName);
      setSearchResults(history.searchResults || []);
      // 清理 state，避免重复加载
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      // 保存到历史记录
      saveToHistory(query, data.response, provider, modelName, searchResults);
    } catch (error) {
